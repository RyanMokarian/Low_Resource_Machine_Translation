#
# Most of the code comes from a tensorflow tutorial (https://www.tensorflow.org/tutorials/text/transformer)
...
class Transformer(tf.keras.Model):
    ...
    def forward(self, inputs, targets, training):
        ...
        dec_output, attention_weights = self.decoder(targets, enc_output, training, combined_mask, dec_padding_mask)
    ...
    def get_name(self):
        ...
        name += f'-num_layers_{self.config["num_layers"]}'
        name += f'-d_model_{self.config["d_model"]}'
        name += f'-num_heads_{self.config["num_heads"]}'
